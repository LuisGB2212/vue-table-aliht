<template>
  <div class="vtt-scope vtt-root vtt-bg-white vtt-rounded-2xl vtt-border vtt-border-neutral-200 vtt-shadow-table vtt-overflow-hidden vtt-flex vtt-flex-col">
    ...
    <div
      ...
      class="vtt-overflow-auto vtt-flex-1"
...
// ── Local mutable columns (for visibility toggling) ──
...
// ── Display mode (local state — user controls it) ────
const currentMode = ref<DisplayMode>(props.displayMode)
watch(() => props.displayMode, m => {
  currentMode.value = m
...
function setupObserver() {
  ...
  if (currentMode.value !== 'infinite' || !sentinelRef.value) return

  observer = new IntersectionObserver(
    (entries) => {
      if (entries[0].isIntersecting) {
        store.loadNextPage(store.activeKey)
      }
    },
    {
...
// Re-setup observer whenever mode or sentinel changes
watch([currentMode, sentinelRef], () => {
  setupObserver()
}, { flush: 'post' })
